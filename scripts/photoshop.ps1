...
function Get-PhotoshopProductName {
    param([Parameter(Mandatory)][version] $Version)
    # The name on the box, which is also the name of the preferences folder:
    # 17 -> CC 2015.5, 18 -> CC 2017, 19 -> CC 2018, 20 -> CC 2019, and from
    # 21 -> 2020 on it is simply the year.
    switch ($Version.Major) {
        17 { return 'CC 2015.5' }
        18 { return 'CC 2017' }
        19 { return 'CC 2018' }
        20 { return 'CC 2019' }
        default { return [string]($Version.Major + 1999) }
    }
}
...
.OUTPUTS
Objects with Path, Version, Name, Variant, Supported, CepDir, GeneratorDir,
CepTarget and GenTarget.
#>
function Get-PhotoshopInstallations {
    ...
    foreach ($path in $candidates) {
        ...
        # Which panel build the CEP inside this Photoshop can run; the header
        # of scripts/build.mjs has the table. Anything older than CC 2015.5
        # (17.0) runs Generator on Node 0.10, which cannot load the capture
        # plug-in at all, so nothing is offered for it.
        $variant = if ($version.Major -le 18) { 'classic' } elseif ($version.Major -le 21) { 'legacy' } else { 'modern' }
        ...
        $results.Add([pscustomobject]@{
            Path         = $path
            Version      = $version
            Name         = Get-PhotoshopProductName -Version $version
            Variant      = $variant
            Supported    = ($version.Major -ge 17)
            CepDir       = $cepDir
...
function Get-PhotoshopPreferenceHints {
    ...
    $prefsRoot = Join-Path $env:APPDATA "Adobe\Adobe Photoshop $($Installation.Name)\Adobe Photoshop $($Installation.Name) Settings"
