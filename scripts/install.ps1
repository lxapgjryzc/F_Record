<#
.SYNOPSIS
Installs F_Record into every Photoshop from CC 2015.5 to 2026 on this machine.
...
matching panel build plus the capture plug-in into it:
    ...
    Photoshop CC 2015.5..CC 2017  -> the 'classic' panel  (CEP 6.1/7, Chromium 41)
    Photoshop CC 2018..2020       -> the 'legacy' panel   (CEP 8/9, Chromium 57/61)
    Photoshop 2021..2026          -> the 'modern' panel   (CEP 10/11/12)

Older Photoshops (CC 2015.1 and before) run Generator on Node 0.10, which
cannot load the capture plug-in; they are listed and skipped.
...
build will load. The repository's build script produces unsigned release panels,
so those packages need this option unless PlayerDebugMode is already enabled.
...
foreach ($ps in $installations) {
    $note = if ($ps.Supported) { "-> $($ps.Variant) build" } else { 'unsupported (needs CC 2015.5 or newer)' }
    Write-Host ("  {0,-10} {1,-8} {2}" -f $ps.Name, $ps.Version, $note)
...
foreach ($ps in $installations) {
    Write-Section "Photoshop $($ps.Name)  ($($ps.Version))"

    if (-not $ps.Supported) {
        Write-Host '  Skipped: F_Record requires Photoshop CC 2015.5 (17.0) or newer.' -ForegroundColor Yellow
        Write-Host '  Older releases run Generator on Node 0.10, which cannot load the capture plug-in.' -ForegroundColor DarkGray
        continue
    }
...
if ($DevMode) {
    ...
    # CSXS 6..12 covers Photoshop CC 2015.5 through 2026.
    foreach ($n in 6..12) {
