...
foreach ($ps in $installations) {
    ...
    Write-Host ''
    Write-Host ("  Photoshop {0}  (version {1})" -f $ps.Name, $ps.Version) -ForegroundColor White
    ...
    if ($panelOk) {
        # The build stamps which variant it is. An install from before the
        # stamp existed reads as unknown rather than being guessed at.
        $installedVariant = 'unknown'
        $buildInfo = Join-Path $ps.CepTarget 'build.json'
        if (Test-Path -LiteralPath $buildInfo) {
            try {
                $installedVariant = [string](Get-Content -LiteralPath $buildInfo -Raw | ConvertFrom-Json).variant
            } catch { }
        }
...
foreach ($ps in $installations) {
    $psLogDir = Join-Path $env:APPDATA "Adobe\Adobe Photoshop $($ps.Name)\Logs"
