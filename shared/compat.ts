...
/** A plain byte copy, for when a link would be wrong -- a move across drives, say. */
...
/**
 * The user's home directory on a Node that may predate `os.homedir` (2.3):
 * the CEP 7 panel of Photoshop CC 2015.5/CC 2017 runs io.js 1.2. Windows
 * always has the profile in the environment, and HOME is what os.homedir
 * reads first elsewhere.
 */
export function homeDirectory(env: NodeJS.ProcessEnv, homedir: (() => string) | undefined): string {
...
export function getUserDataDir(): string {
    const home = homeDirectory(process.env, os.homedir);
...
/**
 * `Buffer.from`, `Buffer.alloc` and `Buffer.allocUnsafe` on a Node that
 * predates them (4.5 and 5.10). Photoshop CC 2015.5 and CC 2017 run Generator
 * on Node 4.3.1, and the io.js 1.2 in their panel is older still.
 *
 * On Node 4 the name `Buffer.from` resolves to the inherited `Uint8Array.from`,
 * which takes no encoding and returns no Buffer, so presence alone is not the
 * test; this is the same check Node itself used when it added the real one.
 * `alloc` takes its fill as a byte value only, which is all this code base
 * ever passes.
 */
export function installBufferShims(B: any): void {
    if (!B) {
        return;
    }
    if (typeof B.from !== "function" || B.from === Uint8Array.from) {
        B.from = function (value: any, encodingOrOffset?: any, length?: any): any {
            return new B(value, encodingOrOffset, length);
        };
    }
    if (typeof B.alloc !== "function") {
        B.alloc = function (size: number, fill?: any): any {
            // The old constructor hands back whatever was in memory, so an
            // alloc with no fill has to zero it by hand.
            return new B(size).fill(fill === undefined ? 0 : fill);
        };
    }
    if (typeof B.allocUnsafe !== "function") {
        B.allocUnsafe = function (size: number): any {
            return new B(size);
        };
    }
}
