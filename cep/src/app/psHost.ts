...
/**
 * The panel's own stylesheet as text, for the var() fallback on Chromium 41.
 *
 * Read from disk rather than from document.styleSheets, which on that host
 * has already thrown every var() declaration away. Null when the host will
 * not say where the extension lives; the fallback then simply does nothing.
 */
export function readPanelStylesheet(): string | null {
    try {
        const extensionRoot = csi().getSystemPath("extension");
        return fs.readFileSync(path.join(extensionRoot, "panel.css"), "utf8");
    } catch (e) {
        return null;
    }
}
...
/**
 * Opens a file in this Photoshop, or brings it to the front if it is already
 * open. "missing" means nothing is at that path any more.
 */
export async function openDocumentInPhotoshop(filePath: string): Promise<"ok" | "missing"> {
    const result = await evalScript("$.f_record.openDocument('" + encodeArg(filePath) + "')");
    if (result === "ok" || result === "missing") {
        return result;
    }
    throw new Error(result.replace(/^error:/, "") || "Could not open the document");
}

export type ReviewOpenOutcome = "opened" | "already" | "missing";

/**
 * openDocumentInPhotoshop, but says whether the file was opened or was open
 * already; the clean-up review closes behind itself only what it opened.
 */
export async function openDocumentForReview(filePath: string): Promise<ReviewOpenOutcome> {
    const result = await evalScript("$.f_record.openForReview('" + encodeArg(filePath) + "')");
    if (result === "opened" || result === "already" || result === "missing") {
        return result;
    }
    throw new Error(result.replace(/^error:/, "") || "Could not open the document");
}

export type CloseOutcome = "ok" | "not-open" | "cancelled";

/**
 * Closes the document at `filePath` if it is open. `discard` drops unsaved
 * changes without asking; otherwise Photoshop asks, and the user may cancel.
 */
export async function closeDocumentInPhotoshop(filePath: string, discard: boolean): Promise<CloseOutcome> {
    const result = await evalScript(
        "$.f_record.closeDocument('" + encodeArg(filePath) + "', " + (discard ? "true" : "false") + ")"
    );
    if (result === "ok" || result === "not-open" || result === "cancelled") {
        return result;
    }
    throw new Error(result.replace(/^error:/, "") || "Could not close the document");
}

export type SwitchOutcome = "ok" | "missing" | "cancelled";

/**
 * Saves and closes whatever is in front, then opens `filePath`; see
 * switchToDocument in init.jsx. "cancelled" means the user backed out of a
 * dialog along the way and nothing was closed or opened.
 */
export async function switchToDocumentInPhotoshop(filePath: string): Promise<SwitchOutcome> {
    const result = await evalScript("$.f_record.switchToDocument('" + encodeArg(filePath) + "')");
    if (result === "ok" || result === "missing" || result === "cancelled") {
        return result;
    }
    throw new Error(result.replace(/^error:/, "") || "Could not switch documents");
}
...
/** Picks a still for the watermark. PNG first: it is the one that keeps alpha. */
export function chooseImageFile(title: string, initial: string): string | null {
    try {
        const result = window.cep.fs.showOpenDialog(false, false, title, initial, [
            "png",
            "jpg",
            "jpeg",
            "webp",
