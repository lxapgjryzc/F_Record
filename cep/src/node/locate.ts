...
/** Point this at a .ttf/.ttc to draw the text watermark in a font of your own. */
export const FONT_ENV_VAR = "F_RECORD_FONT";
...
/**
 * Fonts to draw a text watermark with, best first.
 *
 * ffmpeg's drawtext needs a font *file*, not a family name, so the list is of
 * concrete files that Windows and macOS ship with. CJK-capable faces come
 * first deliberately: a Latin-only font renders Chinese, Japanese and Korean
 * as empty boxes, and this panel speaks ten languages -- whereas a CJK font
 * draws Latin perfectly well, so nobody loses by that order.
 */
export function fontCandidates(ctx: LocateContext): string[] {
    ...
    if (platform === "win32") {
        ...
        add(join(platform, fonts, "msyh.ttc")); // Microsoft YaHei
        add(join(platform, fonts, "msyh.ttf"));
        add(join(platform, fonts, "simhei.ttf"));
        add(join(platform, fonts, "meiryo.ttc"));
        add(join(platform, fonts, "malgun.ttf"));
        add(join(platform, fonts, "segoeui.ttf"));
        add(join(platform, fonts, "arial.ttf"));
    } else {
        add("/System/Library/Fonts/PingFang.ttc");
        add("/System/Library/Fonts/Hiragino Sans GB.ttc");
        add("/System/Library/Fonts/Helvetica.ttc");
        add("/Library/Fonts/Arial.ttf");
