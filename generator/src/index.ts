...
import { frameFileName, sessionFolder } from "../../shared/paths";
import { describeNodeCompat, installBufferShims, isDirectory, mkdirp, randomHex } from "../../shared/compat";
import { deleteSessions, planPack } from "./housekeeping";
import { Trash, trashFiles } from "./trash";
...
// Before anything encodes a frame: Photoshop CC 2015.5 and CC 2017 run this on
// Node 4.3.1, where Buffer.alloc does not exist and Buffer.from is the wrong
// function. A no-op on every later host.
...
interface PluginOptions {
    syncStallMs?: number;
    /** Where deleted documents go; the tests pass a bin that does not fill the real one. */
    trash?: Trash;
}
...
class FRecordPlugin {
    ...
    /**
     * Folders on their way somewhere else, by session id. A session in here
     * is neither captured into nor has its manifest flushed until it has
     * arrived; see startMove.
     */
    private readonly moves: { [sessionId: string]: MoveProgress } = {};
    /** Zips being written, by session; see startPack. */
    private readonly packs: { [sessionId: string]: MoveProgress } = {};
    ...
    constructor(
        ...
        this.trash = typeof options.trash === "function" ? options.trash : trashFiles;
    ...
    private async performCapture(): Promise<void> {
        ...
        // whichever of the two locations the timing chose. Skipped rather
        // than queued: the next stroke asks for a capture of its own.
        if (this.moves[session.sessionId]) {
            return;
        }
        ...
        if (session.manifest.archived) {
            // Drawing on it again is the one sure sign it was not finished.
            applyArchived(session.manifest, false);
            this.log.info("Session " + session.sessionId + " is out of the archive: a new frame was recorded");
        }
        this.lastFrameAt = at;
    ...
    private flushManifest(): void {
        const session = this.current;
        if (!session || this.moves[session.sessionId]) {
            // writeManifest creates the folder it writes into. Landing after
            // a move has emptied the old location would leave a stub folder
            // behind carrying this session's id -- which locateSession would
            // then prefer over the real one.
            return;
        }
        try {
    ...
    /** The listing, with any move or pack under way noted on its row. */
    private listSessions(config: Config): SessionSummary[] {
        const sessions = listSessionsOnDisk(config.processImageFolderPath);
        for (let i = 0; i < sessions.length; i++) {
            const move = this.moves[sessions[i].sessionId];
            if (move) {
                sessions[i].moving = { done: move.done, total: move.total, to: move.to };
            }
            const pack = this.packs[sessions[i].sessionId];
            if (pack) {
                sessions[i].packing = { done: pack.done, total: pack.total, to: pack.to };
            }
        }
        return sessions;
    }

    /** Why a recording is off limits to a move, an archive flag or a delete right now. */
    private busyWith(sessionId: string): string | null {
        if (this.moves[sessionId]) {
            return "This recording is being moved; try again once it has arrived";
        }
        if (this.packs[sessionId]) {
            return "This recording is being packed; try again once its zip is written";
        }
        return null;
    }

    /** busyWith, plus the take in progress, which the bulk actions never touch. */
    private refuseBulk(sessionId: string): string | null {
        if (this.current && this.current.sessionId === sessionId) {
            return "This recording is in progress; close its document first";
        }
        return this.busyWith(sessionId);
    }

    /** The title a warning names a recording by: what the panel shows, not the id. */
    private nameOf(config: Config, sessionId: string): string {
        const folder = locateSession(config.processImageFolderPath, sessionId);
        const manifest = folder ? readManifest(folder) : null;
        return manifest && manifest.docName ? manifest.docName : sessionId;
    }

    /**
     * Writes each recording into a zip of its own, one after another.
     *
     * Returns as soon as the batch is queued, as startMove does: a long
     * recording with its PSD is gigabytes, and the panel's request would
     * give up long before the first zip was done. Every recording in the
     * batch is flagged `packing` at once, so the panel's poll sees the
     * queue, and each is unflagged as its zip lands. A recording whose zip
     * fails is left exactly as it was, and the failure goes to the log,
     * which the panel shows.
     */
    private startPack(config: Config, sessionIds: string[], folder: string, deleteAfter: boolean): CommandResult {
        const root = config.processImageFolderPath;
        if (typeof folder !== "string" || folder.length === 0) {
            return { ok: false, error: "No folder to write the zips into" };
        }
        try {
            mkdirp(folder);
        } catch (e) {
            return { ok: false, error: "Cannot write to '" + folder + "': " + errText(e) };
        }
        if (!isDirectory(folder)) {
            return { ok: false, error: "'" + folder + "' is not a folder" };
        }
        ...
        for (let i = 0; i < sessionIds.length; i++) {
            ...
            const reason = this.refuseBulk(sessionId);
            if (reason) {
                warnings.push(this.nameOf(config, sessionId) + ": " + reason);
                continue;
            }
            if (!locateSession(root, sessionId)) {
                warnings.push(this.nameOf(config, sessionId) + ": no longer exists");
                continue;
            }
            this.packs[sessionId] = { done: 0, total: 0, to: folder };
            accepted.push(sessionId);
        }
        const sessions = this.listSessions(config);
    ...
    private async runPack(config: Config, sessionIds: string[], folder: string, deleteAfter: boolean): Promise<void> {
        const root = config.processImageFolderPath;
        for (let i = 0; i < sessionIds.length; i++) {
            const sessionId = sessionIds[i];
            const progress = this.packs[sessionId];
            try {
                const plan = planPack(root, sessionId, folder);
                progress.to = plan.zip;
                progress.total = plan.entries.length;
                await writeZip(plan.zip, plan.entries, (done, total) => {
                    progress.done = done;
                    progress.total = total;
                });
                this.log.info(
                    "Packed session " + sessionId + " into '" + plan.zip + "' (" + plan.entries.length +
                        " files" + (plan.document ? ", with " + plan.document : "") + ")"
                );
                if (deleteAfter) {
                    // Unflagged first: the delete refuses a recording being packed.
                    delete this.packs[sessionId];
                    const outcome = await deleteSessions(
                        root,
                        [{ sessionId: sessionId, withDocument: true }],
                        this.listSessions(config),
                        this.trash,
                        (id) => this.refuseBulk(id)
                    );
                    this.forgetDeleted(outcome.deleted);
                    for (let w = 0; w < outcome.warnings.length; w++) {
                        this.log.warn("After packing: " + outcome.warnings[w]);
                    }
                    if (outcome.deleted.length > 0) {
                        this.log.info("Deleted session " + sessionId + " now that it is packed");
                    }
                }
            } catch (e) {
                this.log.error("Could not pack recording " + this.nameOf(config, sessionId) + ": " + errText(e));
            }
            delete this.packs[sessionId];
        }
    }

    /** Sets or clears the archive flag, on the live manifest when it is the take in progress. */
    private setArchived(folder: string, sessionId: string, archived: boolean): void {
        if (this.current && this.current.sessionId === sessionId) {
            // The live manifest is written over the file every couple of
            // seconds; flag that copy, or the flag would be undone.
            applyArchived(this.current.manifest, archived);
            this.flushManifest();
        } else {
            markArchived(folder, archived);
        }
    }

    private forgetDeleted(sessionIds: string[]): void {
        if (sessionIds.length === 0) {
            return;
        }
        for (let i = 0; i < sessionIds.length; i++) {
            this.index.remove(sessionIds[i]);
        }
        this.index.persist();
    }

    /**
     * Carries a recording's folder to sit beside its document, or back home.
     *
     * Returns as soon as the move is under way rather than when it is done: a
     * rename is instant, but a copy across drives can take minutes for a long
     * recording, and the panel's request would give up long before. Progress
     * travels in the listing instead -- see `moving` -- and the panel keeps
     * asking while anything is on the move.
     *
     * The recording in progress is never moved: the encoder is writing into
     * it. Close the document first, which the switch button does anyway.
     */
    private startMove(config: Config, sessionId: string, destination: MoveDestination): CommandResult {
        const root = config.processImageFolderPath;
        if (this.current && this.current.sessionId === sessionId) {
            return { ok: false, error: "This recording is in progress; close its document first" };
        }
        if (this.moves[sessionId]) {
            return { ok: false, error: "This recording is already being moved" };
        }
        const busy = this.busyWith(sessionId);
        if (busy) {
            return { ok: false, error: busy };
        }
        const from = locateSession(root, sessionId);
        if (!from) {
            return { ok: false, error: "Session '" + sessionId + "' no longer exists" };
        }
        ...
        if (destination === "root") {
            ...
            to = chooseDocumentSideFolder(documentPath, sessionId);
        }
        if (normalizePath(from) === normalizePath(to)) {
            return { ok: true, sessions: this.listSessions(config) };
        }

        const progress: MoveProgress = { done: 0, total: 0, to: to };
        this.moves[sessionId] = progress;
        // Listed before the move starts, not after. A rename is synchronous
        // and lands before the pointer is written, so a listing taken in
        // between finds the folder at neither address and the row vanishes
        // from the panel until the next refresh. Taken now, the row is where
        // it was, flagged as moving, and the panel polls from there.
        const sessions = this.listSessions(config);
        this.log.info("Moving session " + sessionId + " from '" + from + "' to '" + to + "'");
        moveFolder(from, to, (done, total) => {
            progress.done = done;
            progress.total = total;
        })
            .then(() => {
                if (destination === "root") {
                    removePointer(root, sessionId);
                } else {
                    writePointer(root, sessionId, to);
                }
                const entry = this.index.find(sessionId);
                if (entry) {
                    entry.folder = to;
                    this.index.persist();
                }
                // The document may have been opened while the folder was in
                // transit, in which case the session was installed with the
                // old location. Point it at the new one before captures resume.
                if (this.current && this.current.sessionId === sessionId) {
                    this.current.folder = to;
                }
                this.log.info("Moved session " + sessionId + " to '" + to + "'");
            })
            .catch((e) => {
                this.log.error("Could not move recording " + sessionId + " to '" + to + "': " + errText(e));
            })
            .then(() => {
                delete this.moves[sessionId];
            });
    ...
    private async handleCommand(command: Command): Promise<CommandResult> {
        ...
        switch (command.type) {
            ...
            case "deleteSession": {
                const busy = this.busyWith(command.sessionId);
                if (busy) {
                    return { ok: false, error: busy };
                }
            ...
            case "setArchived": {
                ...
                const folder = locateSession(config.processImageFolderPath, command.sessionId);
                if (!folder) {
                    return { ok: false, error: "Session '" + command.sessionId + "' no longer exists" };
                }
                this.setArchived(folder, command.sessionId, command.archived);
                this.log.info((command.archived ? "Archived session " : "Unarchived session ") + command.sessionId);
                return { ok: true, sessions: this.listSessions(config) };
            }
            ...
            case "setArchivedMany": {
                const warnings: string[] = [];
                let flagged = 0;
                const ids = Array.isArray(command.sessionIds) ? command.sessionIds : [];
                for (let i = 0; i < ids.length; i++) {
                    const busy = this.busyWith(ids[i]);
                    if (busy) {
                        warnings.push(this.nameOf(config, ids[i]) + ": " + busy);
                        continue;
                    }
                    const folder = locateSession(config.processImageFolderPath, ids[i]);
                    if (!folder) {
                        warnings.push(ids[i] + ": no longer exists");
                        continue;
                    }
                    try {
                        this.setArchived(folder, ids[i], command.archived);
                        flagged++;
                    } catch (e) {
                        warnings.push(this.nameOf(config, ids[i]) + ": " + errText(e));
                    }
                }
                this.log.info((command.archived ? "Archived " : "Unarchived ") + flagged + " session(s)");
                return { ok: true, sessions: this.listSessions(config), warnings: warnings };
            }

            case "deleteSessions": {
                const items = Array.isArray(command.items) ? command.items : [];
                const outcome = await deleteSessions(
                    config.processImageFolderPath,
                    items,
                    this.listSessions(config),
                    this.trash,
                    (id) => this.refuseBulk(id)
                ...
                for (let i = 0; i < items.length; i++) {
                    if (items[i].withDocument && outcome.deleted.indexOf(items[i].sessionId) !== -1) {
                        withDocuments++;
                    }
                }
                this.log.info(
                    "Deleted " + outcome.deleted.length + " session(s)" +
                        (withDocuments > 0 ? ", " + withDocuments + " with the document" : "")
                );
                return { ok: true, sessions: this.listSessions(config), warnings: outcome.warnings };
            }

            case "packSessions":
                return this.startPack(
                    config,
                    Array.isArray(command.sessionIds) ? command.sessionIds : [],
                    command.folder,
                    !!command.deleteAfter
                );

            case "moveSession":
                return this.startMove(config, command.sessionId, command.destination);
