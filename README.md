...
当前版本为 **4.0.1**，面向 **Windows、Photoshop CC 2015.5–2026（17.x–27.x）**。安装包包含后台生成器和三种 CEP 面板，安装器按 Photoshop 版本自动选择。

## 安装与升级

1. 下载 `F_Record-4.0.1.zip` 并完整解压。GitHub 自动生成的 Source code 压缩包是源码，需要先构建。
...
升级使用同一命令，同时更新面板和生成器。两者的协议版本为 **13**，应成套安装。
...
打开面板的录制开关后，插件跟随当前文档保存过程图。录制在后台执行，关闭或隐藏面板仍可继续；设置中可开启随 Photoshop 启动自动录制。
...
构建输出到 `dist\`，发布包输出到 `release\F_Record-4.0.1.zip`。开发环境使用系统 Node.js，插件运行时使用 Photoshop 自带的 Node.js。
