# F_Record

[English](./README_EN.md)

一款用来录制绘画过程的轻量级 PS 插件。画布一有变化就抓一帧，最后连成录像。

**当前版本**：4.0（对 3.x 的彻底重写）
**支持系统**：Windows
**支持 PS 版本**：Photoshop 2020 – 2026（21.x – 27.x）

---

## 安装

1. 下载并解压发布包。
2. 双击 `scripts\install.cmd`（会请求管理员权限，因为 Photoshop 装在 Program Files 下）。

   安装脚本会自己找出机器上所有的 Photoshop，读出每一个的真实版本，然后装上对应的那份构建 ——
   PS 2020 用 legacy 版，2021 及以后用 modern 版。不需要你手动拷贝文件夹。

   **关于 ffmpeg**：导出视频要用到 ffmpeg，但它不再塞在安装包里（那是个 138 MB 的文件）。
   安装脚本会先在你机器上找 —— PATH、winget、chocolatey、常见目录都会看一遍 ——
   找到就直接用；找不到才会从
   [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds/releases)
   下载最新的稳定版，校验 SHA-256 之后放到 `%ProgramData%\F_Record\ffmpeg\`。

   下载失败（比如没网）不会让安装中断：面板和录制照常装好，只是导出用不了，
   脚本会明确告诉你。你自己装好 ffmpeg 之后再跑一次安装脚本就行。
   不想让它碰网络就加 `-SkipFfmpeg`。

   想指定某一个 Photoshop，或者先看看它打算做什么：

   ```bash
   powershell -ExecutionPolicy Bypass -File scripts\install.ps1 -WhatIf
   ```

   ```bash
   powershell -ExecutionPolicy Bypass -File scripts\install.ps1 -Path "D:\Adobe Photoshop 2024"
   ```

3. 完全退出 Photoshop 再重新打开。
4. 检查「编辑 ▸ 首选项 ▸ 增效工具」：
   - **启用生成器** 要勾上（真正干录制活的是它）
   - **载入扩展面板** 要勾上（在「旧版扩展」那一栏）

   有任何一个是你刚勾的，就再重启一次 Photoshop。

5. 面板在「窗口 ▸ 扩展（旧版）▸ F_Record」。

出问题就跑诊断脚本，它会告诉你卡在哪一步：

```bash
powershell -ExecutionPolicy Bypass -File scripts\doctor.ps1
```

卸载：双击 `scripts\uninstall.cmd`。你的录像放在 `%APPDATA%\F_Record`，卸载不会动它。

---

## 使用

**开关打开，然后就可以不管了。** 录制是在 Photoshop 的后台进程里跑的，面板关掉、藏起来都不影响。
在设置里打开「启动 Photoshop 时自动开始录制」，以后连开关都不用碰。

每个文档的过程图存在各自的文件夹里。**关掉重开、甚至重启 Photoshop，都会接着录进原来那个
文件夹** —— 这是 4.0 修掉的主要问题，下面有说明。另存为则会分家：新文件拿到一份到此为止的
完整录制，留在盘上的旧文件保留原来那份。

「录像」标签页列出所有历史录制，任何一段都能单独导出，不只是当前打开的这个文档。
保存过的文档还能从这里直接在 Photoshop 里打开（「打开 PSD」按钮，已经开着就切到前台）；
录制开着的话会接着录进原来那个文件夹。「切换」则先保存并关闭当前文档再打开它，
适合一次只开一张画布的画法。按钮都是图标，鼠标停上去有文字。

画完的作品点「归档」挪到「归档」标签页，录像列表只留还在画的；再画一笔它会自动回来。
正在录的那段固定在列表最上面。「回形针」把过程图文件夹搬到 PSD 旁边（`dragon.psd`
旁边多一个 `dragon_frames`），再点一下搬回来；搬走的录像照样能列出、续录、导出和删除。

攒多了要清仓库的时候，「录像」页的「归档旧录像 (N)」一键把帧数不足或创建太久的录像全部归档
（门槛在设置里调）。「归档」页可以勾选：「选中旧录像」按同样的规则一键勾出来，勾好的可以一次
全部在 Photoshop 里打开、打包成 zip（每段一个，`dragon.psd` 加 `dragon_frames/`，可选打完就删）、
删除录像，或者连源文件一起删除。「清理…」则是依次在 Photoshop 里打开每段的 PSD，看着画面决定
保留还是删，看完汇总成一次确认。删源文件走回收站，不是直接删。

### 几个设置

| 设置 | 说明 |
|---|---|
| 过程图文件夹 | 默认在 C 盘，建议换到空间够的盘。改了之后已有的录像不会自动搬走。 |
| 分辨率 / 质量 | 越高最终视频越清楚，过程图也越占地方。估算占用别拿空白画布估，jpg 的大小跟画面复杂度关系很大。 |
| 抓帧间隔 | 两帧之间的最短间隔。这只是下限：真实间隔会根据抓帧实际耗时自动往上调，画得越重的文档抓得越稀。 |
| 离开时间 | 超过这个时间没落笔就停止计时，用时统计不会把发呆算进去。 |
| 旧录像 | 「归档旧录像」和「选中旧录像」的门槛：帧数不足多少、创建超过多少天，满足任意一条即可。默认 20 帧、30 天，设成「关」就不看那一条。 |
| 水印 | 给导出的视频、以及复制到剪贴板的画面盖上署名或者 logo。见下面的导出一节。 |
| 复制画面的分辨率 | 「复制画面」放进剪贴板的图有多大，口径和录制分辨率一样。默认 1080p，要全尺寸选「原图」。见下面的复制画面一节。 |
| 语言 | 默认「跟随 Photoshop」，也可以手动指定。见下面的语言一节。 |
| 检查更新 | **默认关闭。** 打开后大约每天问一次 GitHub 有没有新版本。 |

### 导出

导出前可以挑画幅比例和成片时长。播放一律匀速：每帧时长相同，和 3.x 一样。

这两项会记着上一次导出选的，下次打开对话框就停在那儿。要是这段录像短到给不出那个时长
（比如上次选了 30 秒，这次的录像总共才 3 秒），时长就退回「原长」，比例照旧。

选了比原片短的时长（比如 30 秒），就按比例抽帧来提速 —— 帧被丢掉，而不是被压短，
所以画面不会抖。首帧和末帧一定保留，片子还是停在成品上。
早先版本里还有个「真实节奏」选项（按落笔间隔播放，保留停顿），已经去掉了。

**水印**：可以是一行文字（署名、ID）或者一张图片（logo、手写签名的 PNG）。
在设置里选类型、填内容，再挑样式、大小和不透明度；大小是相对视频高度的百分比，
所以换分辨率导出不用重调。导出对话框里能临时换文字或者换图，也能这一次不盖 ——
改的只是这一次，设置里存的不动。

样式有两种。**角标**是一枚水印停在某个角上（或者居中），位置在设置里挑。**浮雕满屏**
把水印斜着平铺满整个画幅，画成一层浮雕：只有高光和暗部，像压进纸里的钢印，没有位置
可挑，因为它铺的是整幅画面。这时的「大小」是单枚水印的高度，2%–8% 比较合适。

浮雕用的就是 PS 里那套「中性灰图层 + 混合模式」，只是混合模式换掉了：PS 的「叠加」在
纯白和纯黑上算出来等于没动，而过程图恰恰是白底黑线，盖上去几乎看不见。这里用的是相加
式的混合（ffmpeg 的 `grainmerge`），白纸上和浓墨上都读得出来。整层浮雕每帧都一样，
所以只画一次就冻住，导出速度和不盖水印没有区别。

浮雕的「不透明度」和角标不是一回事：相加式的混合里，这个数直接就是把像素推开多远，
100% 会推开半个色阶范围 —— 那是印在画上的黑白戳，不是压进纸里的印子。所以这一档的
百分比整个映射到这种样式真正用得上的那一段：100% 是防盗那头（明明白白在那儿，但仍然
是浮雕），30% 只剩一层若有若无的印子。视频拍的是画，水印不该盖过它。

水印盖在最后一步，片头和片尾的成品静帧上也有。角标的文字是白字加黑描边，因为过程图是
拼在白底上的，纯白的字在浅色画面上等于没有。文字用系统字体画（Windows 上优先微软雅黑，
中日韩都能显示），想指定别的字体就把环境变量 `F_RECORD_FONT` 指到一个 .ttf/.ttc。
PNG 的透明部分会保留，不会变成一个半透明方块；浮雕满屏下透明的地方就是不起伏的地方，
所以抠好的 logo 压出来是它自己的形状，不是一个方块。

### 复制画面

录制页面「导出」旁边的**复制画面**，把当前画布盖上水印直接放进剪贴板，可以在聊天窗口里
直接粘出来。不经过录像：没开录、一帧都还没抓，只要有打开的文档就能点。

盖的是设置里存的那份水印（样式、位置、大小、不透明度都照旧），画的也是导出视频那套
滤镜，所以粘出来的图和成片上的签名是同一个东西。

**盖不盖在设置里定**：水印那一栏底下有个「复制画面时加水印」的开关，关掉就是复制原图，
导出视频照旧盖。做成设置而不是每次问，是因为这个按钮的意义就在于一次点击 —— 导出是
个郑重的动作，本来就有对话框可以问；画到一半复制一张，答案基本每次都一样。水印本身
设成「关」的时候这个开关不显示，没有可加的东西。

**默认不是原图。** 这个按钮是拿来随手发进度的，而剪贴板里放的是没压缩的位图：一张
6000 像素的画布就是上百 MB，粘到哪个聊天窗口都要卡一下。剪贴板里只有像素，JPEG 质量那种
压缩在这儿不起作用，能让它变小的只有像素数。所以设置里有一项「复制画面的分辨率」，口径和
录制分辨率完全一样：像素数大约等于这个高度的 16:9 一帧，画布本来就更小的照原样复制，选
1080p 复制出来的就是一帧 1080p 录像那么大。默认 1080p，不放大看不出差别；要全尺寸的图
就选「原图」。缩放在盖水印之前做，水印按缩完的图算大小，和成片上的比例一致。

复制的时候会短暂暂停抓帧，免得把这一下当成一帧存进录像里。Windows 上走 PowerShell，
macOS 上走 AppleScript，都是系统自带的，不用另装。

---

## 4.1 新增

### 面板支持十种语言

默认跟随 Photoshop 自己的界面语言，认不出来就用英文。也可以在设置里手动指定：

简体中文 · 繁體中文 · English · 日本語 · 한국어 · Deutsch · Français · Español ·
Português (Brasil) · Русский

地区变体会落到最接近的那一个：`zh_HK`、`zh_MO` 走繁体，`zh_SG` 走简体，
`de_AT`、`fr_CA`、`es_MX` 各自落到 de、fr、es，`pt_PT` 落到 pt-BR。
认不出来的语言一律回退英文，而不是显示成 key。

> 除了中英文，其余语言是机翻的，只做了术语和 Photoshop 菜单名的核对。
> 发现别扭的地方欢迎提 issue 或者直接改 `cep/src/app/locales/` 下对应的文件——
> 一个语言一个文件，加一个新语言不用动别的代码。

### 更新提示（默认关闭）

打开之后，大约每天向 GitHub 问一次有没有新版本，有就在面板顶部显示一条提示，
可以关掉；关掉只针对这一个版本，下个版本还会再提示。设置里也有「立即检查」。

**默认是关的**，不打开就完全不联网。打开之后也只是一个不带任何参数的公开接口请求：
不会发送任何关于你、你的文档或你的使用情况的信息。

### 反馈问题

面板底部有一个「反馈问题」按钮，直接打开 GitHub 的 issue 页。

---

## 4.0 改了什么

### 录制期间不再静默卡死

3.x 卡死不是玄学，是四件事叠在一起，都在这一版修了：

1. **每秒两次全量图层遍历，而且关掉开关也照跑。** 旧代码每 500ms 调一次
   `getDocumentInfo()`，用的是默认参数 —— 那会在 Photoshop 主线程上用 ExtendScript
   把每一个图层走一遍。现在改成事件驱动，并且把 `layerInfo` / `compInfo` / `getTextStyles`
   全部关掉，只要 `imageInfo`（就是画布尺寸和文件路径）。
2. **每帧 4 次文档信息 + 2 次 pixmap。** 现在每帧只有 **1 次** pixmap 调用：
   用 `inputRect` + `outputRect` + `clipToDocumentBounds` 三件套让 Photoshop 自己
   裁到画布并按指定尺寸渲染，省掉了旧代码那个 boundsOnly 预取和一整段
   padding/extract 算术。三个参数缺一不可 —— 只给 `maxDimension` 的话
   `clipToDocumentBounds` 会被静默忽略，这正是 4.2.1 及更早那个补白 bug 的根因。
3. **`isGettingImage` 会永久卡在 true。** 旧代码的 `catch (error) { throw error }` 写在复位之前，
   JSON 读写一抛异常，这个标志就再也回不来了，录制静默死亡直到重启 PS。
   现在每次抓帧都有 30 秒看门狗，标志在 `finally` 里无条件复位。
4. **完全没有节流。** generator-core 自己都在日志里写着
   `WARNING the imageChanged event is expensive`，旧代码却对每个事件都抓一次。
   现在有自适应节流：下次间隔 = 上次耗时 × 3（夹在设定下限和 15 秒之间），
   一连串事件只会合并成一次抓帧 + 一次收尾补帧。

连续失败会**自动暂停并在面板上写明原因**，而不是悄悄不动了。

### 另存为不再把录制搞丢

Photoshop 有个很老的 bug：另存为会把文档里的 `generatorSettings` 清空。
3.x 只把录制身份存在那里，所以画到一半另存为，录制就断了，后面的帧进了一个新文件夹。

4.0 把身份同时写在三个地方，三者失效条件互不相同：

| 存放位置 | 扛得住 | 扛不住 |
|---|---|---|
| PSD 的 generatorSettings | 关闭重开 | 另存为 |
| 内存里的 documentId 映射 | 另存为 | PS 重启 |
| 磁盘上的恢复索引 | PS 重启 | 文档改名 |

任意一个都能把文档认回来，而且**一发现 PSD 里那份没了就立刻写回去**。
3.x 里那个手工补丁，变成了自动、永久生效的机制。

恢复索引和前两个不一样：它记的是「哪个文件」，而不是「哪个文档」，路径上现在放着什么它就
认什么。文件名比画重复得多 —— 新画一张存成旧名字、资源管理器里把一个 PSD 复制到另一个上面，
路径都没变，画已经不是那幅画了。所以索引只在两件事都成立时才算数：文档是从那个文件**打开**的
（刚另存到这个路径上的是一件新作品，它自己的录制在前两处），而且画布尺寸和那段录制最后一次
见到的一样。PSD 自己带的 id 永远不会被索引推翻 —— id 是跟着像素走的，文件被复制、改名到哪儿，
里面装的还是那幅画。

三个都对不上时（比如重启 PS 后打开一个陌生文件），会按画布尺寸列出候选让你选，
**绝不会自己认领** —— 认错了会毁掉一段录制，多开一个文件夹只是浪费点空间。

### 另存为会分出两份录制

4.2 之前，另存为之后两个文件指向同一段录制：接着画的那个文档在录，而留在盘上的旧文件里也
还带着同一个 id。回头把旧文件打开试另一条路，两个文档就同时往一个文件夹里写帧，导出来的
视频把两幅画交替剪在一起。

现在另存为会分家。**打开着的那个文档拿到一份到此为止的完整拷贝**，从此单独记；旧文件保留
原来的文件夹，冻结在另存为那一刻。两个文件各有各的完整过程视频，谁也不缺前半段。同一个文件
另存为很多次也一样，每个名字都留下一段属于它自己的录制。

方向不是随便定的：Photoshop 只允许往当前打开的文档写 `generatorSettings`，所以只有它能被
当场盖上新 id；盘上那个文件带着旧 id，也就只能留在原来的文件夹。副作用是文件夹名字正好跟着
文件名走。

拷贝优先用硬链接，所以一万帧的录制分家也几乎不占额外空间、不花时间 —— 两个文件夹依然互相
独立，删掉、改名、导出哪一个都不影响另一个。文件系统不支持硬链接时（exFAT、网络盘）退化成
真正的复制。

顺带把存储方式也改了，让它经得起意外：

- `session.json` 直接放在过程图文件夹**里面**，文件夹搬走、备份、拷到别的机器都不丢元数据。
- 帧文件名是 `<序号>_<时间戳>.jpg`。**没有独立的计数器文件可以失步了** ——
  帧数直接由目录内容决定。3.x 是把 count 存在另一个 JSON 里，而面板又用
  `readDir().length` 另算一遍，两边对不上。

### 版本覆盖到 2020–2026

一份源码出两份构建，因为这一段跨了三代浏览器内核：

| PS | 年份 | CEP | Chromium | CEP 里的 Node |
|---|---|---|---|---|
| 21.x | 2020 | CEP 9 | 61 | 8.6 |
| 22.x | 2021 | CEP 10 | 74 | 12.3 |
| 23.x – 25.11 | 2022–2024 | CEP 11 | 88 | 15.9 |
| 25.12+, 26.x, 27.x | 2024–2026 | CEP 12 | 99 | 17.7 |

所以：

- **面板 UI 不再用 React Spectrum**，改成自绘（Preact + 手写 CSS）。Spectrum 在
  Chromium 61/74 上必然排版错乱：flex `gap` 要 Chrome 84，`:focus-visible` 要 86。
  CSS 按 Chromium 61 的基线写，两份构建只差 JS 的编译目标。
- **加了一层 Node 兼容层。** 3.x 直接调 `fs.rmSync`（需要 Node 14.14），
  在 PS 2020/2021 的面板里必崩 —— 导出功能在那两个版本上其实一直是坏的。
  现在这类 API 全部特性检测 + 回落。

### 两个进程之间改用真正的通道

3.x 是两个进程隔着 JSON 文件互相喊话，各自 500ms 轮询一次。竞态、临时文件残留
（面板里还得专门写代码去删 `configData.json.*`），最要命的是**面板根本分不清
「没在录」和「录制进程已经死了」** —— 两种情况看起来都是数字不动了。

现在 Generator 在 `127.0.0.1` 上开一个只监听回环地址的 HTTP 端点，
用 SSE 把状态推给面板。面板变成纯客户端：它只负责显示和发命令，连不上就明说连不上。

安全上：只绑回环地址、每次运行随机生成 token、并且**拒绝任何带 `Origin` 头的请求** ——
浏览器没法在跨源请求里省掉这个头，而面板走的是 Node 的 http 模块（CEP 面板本来就带 Node），
从不发 Origin，所以这一条能干净地挡掉网页扫本地端口。

### 导出重做

- **不再依赖用户装了 Node.js。** 3.x 用 `spawn("node", ...)` 起一个 worker ——
  只有恰好装了 Node 并且在 PATH 上才跑得通，大多数 PS 用户并没有装，
  导出会以一句莫名其妙的 "Worker exited" 失败。现在直接调 ffmpeg。
- **ffmpeg 不再塞进安装包。** 之前是把一个 75 MB 的 ffmpeg.exe 一起打包，
  于是这个插件自己的代码只有 419 KB，下载包却几乎全是第三方二进制文件，
  仓库也因此被撑到 165 MB。现在安装脚本先在机器上找，找不到才去下载一份。
- **一次 ffmpeg 调用**代替原来的四趟（主视频 .ts + 片头 .ts + 片尾 .ts + concat）。
- **不再把每一帧复制到临时目录**，直接从原位置读。长录制能省掉一半磁盘 IO。
- 损坏的帧（比如崩溃时写了一半）会被跳过，而不是让整个导出失败。

---

## 开发

```bash
npm install
npm run build      # 构建 dist/（generator + legacy 面板 + modern 面板 + 脚本）
npm run check      # 类型检查 + 全部测试
```

测试用 `node --test`，不需要装 Photoshop：会话身份的各种场景、抓帧调度的状态机、
pixmap 通道解码、导出计划的生成、以及 Node 兼容层的回落，全部覆盖。
其中 `test/integration.test.mjs` 会加载**真正构建出来的 generator 产物**，
拿一个 mock 的 Photoshop 驱动它跑完整流程。

```
shared/          两端共用：协议、路径布局、Node 兼容层
generator/src/   抓帧引擎（在 Photoshop 的 Generator 进程里跑）
cep/src/         面板（Preact）和导出管线
scripts/         构建、安装、卸载、诊断
test/            单元测试 + 集成测试
```
