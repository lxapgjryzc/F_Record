...
test("a document that was being recorded at the last quit is recorded again at the next launch", async (t) => {
    ...
    // The document comes back with its session, as the README promises for a
    // restart -- and the switch comes back with it. Auto-start has nothing to
    ...
    while (session.frameCount === 0 && Date.now() < deadline) {
        await settle(1);
        session = (await command(h, { type: "ping" })).state.session;
    }
    assert.equal(session.sessionId, OLD, "the stroke went into the same recording");
...
test("the heartbeat re-syncs the document and offers the panel a fresh state", async (t) => {
    const h = await startPlugin(t, { config: { autoStart: true }, captureTimers: true });
    await settle();
    const before = h.ps.calls.documentInfo.length;
...
test("a recording with no document on disk cannot be moved next to one", async (t) => {
    const h = await startPlugin(t, { config: { autoStart: true }, photoshop: { file: "" } });
    await settle(30);
    const sessionId = await h.sessionId();
    h.ps.emit("imageChanged", { id: 1, closed: true });
    await settle(20);
...
test("a recording already on the move is not moved twice", async (t) => {
    ...
    const h = await startPlugin(t, { config: { autoStart: true }, photoshop: { file: document } });
    await settle();
    const sessionId = await recordedAndClosed(t, h);
...
test("a document Photoshop reports no canvas for is not captured against", async (t) => {
    const h = await startPlugin(t, {
        config: { autoStart: true, minCanvasPixels: 0 },
...
test("a recording whose folder is in transit is not written into, and is repointed after", async (t) => {
    ...
    // A short interval, so the frame below is asked for while the folder is
    // still on its way rather than queued behind the default one.
    const h = await startPlugin(t, {
        config: { autoStart: true, minIntervalMs: 200 },
    ...
    // timing chose.
    h.ps.emit("currentDocumentChanged", { id: 1 });
    // Until the reopened document has its session back. The resync behind
    // the change is debounced, and a frame arriving before it would be
    // dropped for having no session at all, not for the move.
    while ((await command(h, { type: "ping" })).state.session === null) {
        await settle(1);
    }
    const asked = h.ps.calls.pixmap.length;
    h.ps.emit("imageChanged", { id: 1, layers: [{ id: 2, pixels: true }] });
    await settle(30);
    const rows = (await command(h, { type: "listSessions" })).sessions;
    assert.ok(
        rows.some((row) => row.sessionId === sessionId && row.moving),
        "precondition: the folder is still on its way"
    );
    assert.equal(h.ps.calls.pixmap.length, asked, "no frame was even asked for while the folder is in transit");
...
test("captures Photoshop never answers pause recording, and answering again lifts the pause", async (t) => {
    ...
    // Each change is followed by a debounced resync, so wait for the request
    // it sends to actually be in flight rather than for a number of timer
    // ticks: how long a tick takes depends on the machine, and a wait that
    // ends before the debounce does merges the two changes into one sync.
    hang = true;
