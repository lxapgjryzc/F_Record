...
test("the home directory comes from os.homedir where there is one, and from the environment on an io.js without it", () => {
    assert.equal(homeDirectory({ HOME: "/ignored" }, () => "/Users/a"), "/Users/a");
    assert.equal(homeDirectory({ USERPROFILE: "C:\\Users\\a", HOME: "/h" }, undefined), "C:\\Users\\a");
    assert.equal(homeDirectory({ HOME: "/h" }, undefined), "/h");
    assert.equal(homeDirectory({}, undefined), "");
});

test("Buffer.from, alloc and allocUnsafe are supplied on a Node without them, and left alone where they exist", () => {
    // Node 4.3's Buffer: a constructor whose `from` is the one inherited from
    // Uint8Array -- which is why "is there a from" is not the test.
    const made = [];
    function OldBuffer(value, encoding, length) {
        made.push([value, encoding, length]);
        this.value = value;
        this.filled = null;
    }
    OldBuffer.prototype.fill = function (byte) {
        this.filled = byte;
        return this;
    };
    OldBuffer.from = Uint8Array.from;
    installBufferShims(OldBuffer);

    const text = OldBuffer.from("hi", "utf8");
    assert.equal(text.value, "hi");
    assert.deepEqual(made[0], ["hi", "utf8", undefined]);
    // The old constructor hands back whatever was in memory; alloc zeroes it.
    assert.equal(OldBuffer.alloc(4).filled, 0);
    assert.equal(OldBuffer.alloc(4, 0xff).filled, 0xff);
    const raw = OldBuffer.allocUnsafe(8);
    assert.equal(raw.value, 8);
    assert.equal(raw.filled, null);

    // io.js 1.2: no `from` at all.
    function Older(value) {
        this.value = value;
    }
    Older.prototype.fill = function () {
        return this;
    };
    installBufferShims(Older);
    assert.equal(Older.from("x").value, "x");

    // The real thing is not replaced...
    const real = { from: Buffer.from, alloc: Buffer.alloc, allocUnsafe: Buffer.allocUnsafe };
    installBufferShims(Buffer);
    assert.equal(Buffer.from, real.from);
    assert.equal(Buffer.alloc, real.alloc);
    assert.equal(Buffer.allocUnsafe, real.allocUnsafe);
    // ...and a scope with no Buffer in it is not a crash.
    installBufferShims(undefined);
});
