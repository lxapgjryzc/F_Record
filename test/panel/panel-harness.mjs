...
export function stubPanel(mock) {
    ...
    const hostDefaults = {
        ...
        openInExplorer: () => {},
        openUrl: () => {},
        // Read only on a host without native var(); the test DOM has no CSS
        // object, so main.tsx asks and is told there is nothing to read.
        readPanelStylesheet: () => null,
    ...
    const compatExports = {
        ...
        // main.tsx runs the two start-up shims; here there is nothing for
        // either to fill in, so they are the no-ops they would be on a
        // modern host.
        installBufferShims: () => {},
