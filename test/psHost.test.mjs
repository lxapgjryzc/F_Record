...
const host = {
    ...
    pathsAsked: [],
    getSystemPath(kind) {
        this.pathsAsked.push(kind);
        if (this.extensionRoot === null) {
            throw new Error("the host will not say where it lives");
        }
...
test("the panel's stylesheet is read from its own folder, for the var() fallback on Chromium 41", (t) => {
    const temp = tempDir();
    t.after(() => temp.cleanup());
    fs.writeFileSync(path.join(temp.dir, "panel.css"), ":root { --bg: #323232; }");

    host.extensionRoot = temp.dir;
    assert.equal(psHost.readPanelStylesheet(), ":root { --bg: #323232; }");
    assert.deepEqual(host.pathsAsked, ["extension"]);

    // A host that will not say where the extension lives costs the fallback,
    // not the panel.
    host.extensionRoot = null;
    assert.equal(psHost.readPanelStylesheet(), null);
});
