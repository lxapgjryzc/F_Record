...
test("the text watermark reaches for a CJK-capable font first", () => {
    const list = fontCandidates({ platform: "win32", extensionDir: EXT_DIR, env: { WINDIR: w("C:/Windows") } });
    assert.equal(list[0], w("C:/Windows/Fonts/msyh.ttc"), "Latin-only fonts draw Chinese as empty boxes");
    assert.ok(list.indexOf(w("C:/Windows/Fonts/arial.ttf")) > list.indexOf(w("C:/Windows/Fonts/msyh.ttc")));
});

test("a missing WINDIR still yields a usable Windows font list", () => {
    const list = fontCandidates({ platform: "win32", extensionDir: EXT_DIR, env: {} });
    assert.ok(list[0].toLowerCase().indexOf("windows") !== -1);
});

test("an explicit font override wins over everything shipped with the OS", () => {
    const env = { WINDIR: w("C:/Windows") };
    env[FONT_ENV_VAR] = w("D:/fonts/mine.otf");
    const list = fontCandidates({ platform: "win32", extensionDir: EXT_DIR, env: env });
    assert.equal(list[0], w("D:/fonts/mine.otf"));
});

test("posix hosts get posix font paths", () => {
